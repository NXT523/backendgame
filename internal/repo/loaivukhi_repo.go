...
func (r *loaiVuKhiRepoGorm) GetIdGorm(ctx context.Context, id int) (*db.GLoaiVuKhi, error) {
	...
	if err := r.g.WithContext(ctx).First(&row, "ma_loai = ?", id).Error; err != nil {
		return nil, err
	}
	return &row, nil
}
...
func (r *loaiVuKhiRepoGorm) DeleteGorm(ctx context.Context, id int) error {
	return r.g.WithContext(ctx).Delete(&db.GLoaiVuKhi{}, "ma_loai = ?", id).Error
}
...
func (r *loaiVuKhiRepoRaw) CreateRaw(ctx context.Context, in LoaiVuKhiRawInput) (*LoaiVuKhiRaw, error) {
	...
	var nextID int
	if err = tx.QueryRowContext(ctx, `SELECT IFNULL(MAX(ma_loai)+1,1) FROM loai_vu_khi`).Scan(&nextID); err != nil {
		return nil, err
	}
	if _, err = tx.ExecContext(ctx, `
		INSERT INTO loai_vu_khi (ma_loai, ten_loai, mo_ta)
		VALUES (?, ?, ?)`,
...
func (r *loaiVuKhiRepoRaw) DeleteRaw(ctx context.Context, id int) error {
	_, err := r.sql.ExecContext(ctx, `DELETE FROM loai_vu_khi WHERE ma_loai = ?`, id)
	return err
}
